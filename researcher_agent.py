...
def evaluate_researcher_relevance(topic: str, actual_output: str):
    """
    Evaluates the relevance of the found researchers using an LLM Judge.
    Returns a dictionary with score and reason.
    """
    # Define the test case
    # Input: The topic (e.g., "Graph Neural Networks")
    # Actual Output: The list of researchers generated by Agent 1
    test_case = LLMTestCase(
        input=topic, 
        actual_output=actual_output
    )

    # Define the Metric: Researcher Relevance
    # We use GEval to define a custom criteria for this specific task
    relevance_metric = GEval(
        name="Researcher Relevance",
        criteria="Relevance. The list of researchers must be highly influential and directly related to the input research topic.",
        evaluation_params=[LLMTestCaseParams.INPUT, LLMTestCaseParams.ACTUAL_OUTPUT],
        model="gpt-4o" # Or use a custom Gemini wrapper if configured in deepeval, defaults to OpenAI usually
    )
...
def run_evaluation(topic: str, agent_output: str):
    return evaluate_researcher_relevance(topic, agent_output)
