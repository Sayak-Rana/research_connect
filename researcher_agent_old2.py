...
def get_top_researchers(topic: str, top_k: int = 3) -> str:
    ...
    if gemini_api_key:
        ...
        short_names = [name for name, _ in ranked]
        prompt = f"""
Some researcher names below may be abbreviated (e.g., "Y Ding" → "Yong Ding").
For each name, return a JSON mapping from short to full likely name.
Names: {short_names}
"""
        try:
            response = cleanup_agent.run(prompt)
            name_map = json.loads(response.content)
            for i, (name, info) in enumerate(ranked):
                full_name = name_map.get(name, name)
                ranked[i] = (full_name, info)
        except Exception as e:
            print("[WARN] Name cleanup skipped:", e)
...
Paper text (truncated to 2000 words if long):
{paper_text[:2000]}
"""
    ...
    try:
        ...
        print("[WARN] Gemini did not return JSON, fallback to text parsing.")
        main_topic = None
        keywords = []
